...
% I'll need to pass in an over-ride for my parameter searching
% stuff, so that I plot the correct nullclines.

function [meanRate]  = phaseplane_izhi (spineml_results_path, experimentXml, ...
    ...
    %display (['Pop name: ' populationName]);
    ...
    for p_iter = 0:props.getLength-1
        ...
        elseif pname == paramNames('vr')
            vr = str2num(char(property.getElementsByTagName ...
                              ('UL:FixedValue').item (0).getAttribute ...
                              ('value')));
