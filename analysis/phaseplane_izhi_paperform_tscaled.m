...
%
% Finally, time can be scaled with this version using the parameter T.

function [rate, isiRate]  = phaseplane_izhi_paperform_tscaled ...
    ...
    % NB: T is a time scaling parameter.
    a = 0; b = 0; c = 0; d = 0; Vpeak = 0; A = 0; B = 0; C = 0; T=1;
    izhy_cmpt = '';
        ...
        if nm == populationName
            ...
            for p_iter = 0:props.getLength-1
                ...
                elseif pname == paramNames('C')
                    C = str2num(char(property.getElementsByTagName ...
                                         ('FixedValue').item (0).getAttribute ('value')));
                elseif pname == paramNames('T')
                    T = str2num(char(property.getElementsByTagName ...
                                         ('FixedValue').item (0).getAttribute ('value')));
                else
    ...
    % Rescale the A,B,C according to T all at once?
    %A = A.*T;
    %B = B.*T;
    %C = C.*T;
    % And a too?
    ...
    display (['constI: ' num2str(constI)]);
    if (isempty(multiI))
        disp (['constI; T=' num2str(T)]);
    ...
    size(cur_isis)
    cur_isis
    meanISI = mean(cur_isis) * 0.1 % milliseconds - this for a 0.1
                                   % ms timestep in the expt.
